...
if Exoplanet=='YZCet b Model B':
    df_alfven_A=pd.read_csv(FOLDER + '/CSV/' +outfile+'_alfven_wing_model.csv')
    df_reconnect_A=pd.read_csv(FOLDER + '/CSV/' +outfile+'_reconnection_model.csv')  
    outfile_A=outfile
    outfile_A=outfile_A.replace("Model B", "Model A")
    
    df_alfven_A=pd.read_csv(FOLDER + '/CSV/' +outfile_A+'_alfven_wing_model.csv')
    df_reconnect_A=pd.read_csv(FOLDER + '/CSV/' +outfile_A+'_reconnection_model.csv')  
...
ax0 = plt.subplot2grid((2,1),(1,0),rowspan=2,colspan=1)
...
y_A=df_alfven_A['FLUX']


y_min_A = df_alfven_A['FLUX_min'] # minimum flux (array), Saur/Turnpenney model
y_max_A = df_alfven_A['FLUX_max'] # maximum flux (array)
y_inter_A = df_alfven_A['FLUX']

y_min_reconnect_A = df_reconnect_A['FLUX_min']
y_max_reconnect_A = df_reconnect_A['FLUX_max']
y_inter_reconnect_A = df_reconnect_A['FLUX']
  
  
  
y_min_B = df_alfven_B['FLUX_min'] # minimum flux (array), Saur/Turnpenney model
y_max_B = df_alfven_B['FLUX_max'] # maximum flux (array)
y_inter_B = df_alfven_B['FLUX']

y_min_reconnect_B = df_reconnect_B['FLUX_min']
y_max_reconnect_B = df_reconnect_B['FLUX_max']
y_inter_reconnect_B = df_reconnect_B['FLUX']

  
  
ax2.fill_between(x, y_min_A, y_max_A,color="orange", alpha=0.7)
ax2.fill_between(x, y_min_reconnect_A, y_max_reconnect_A,color="blue", alpha=0.7)
ax2.plot(x,y_inter_A,color='black',lw=1.5)
#ax2.plot(x,y_inter,color='orange',lw=3)
ax2.plot(x,y_inter_reconnect_A,color='black',lw=1.5)
...
ax0.fill_between(x, y_min_B, y_max_B, color="orange", alpha=0.7)
ax0.fill_between(x, y_min_reconnect_B, y_max_reconnect_B, color="blue", alpha=0.7)
ax0.plot(x, y_inter_B, color='black', lw=1.5)
#ax0.plot(x, y_inter, color='orange', lw=3)
ax0.plot(x, y_inter_reconnect_B, color='black', lw=1.5)
#ax0.plot(x, y_inter_reconnect, color='blue', lw=3)
if STUDY == 'D_ORB':
    ax0.set_yscale('log') 
    # Draw vertical line at nominal orbital separation of planet
    xnom = r_orb / R_star
    ...
    ax0.set_xlim(1, d_orb_max)
    
    if Bfield_geom_arr[ind] == 'pfss': 
        # ax0.axvline(x = R_SS, ls='--', color='k', lw=2)
        #ax0.axvspan(x[0], R_SS, facecolor='grey', alpha=0.5, hatch='x')
        ax0.axvspan(x[0], R_SS, facecolor='gray', alpha=0.7)
    def tick_function(X):
        V = spi.Kepler_P(M_star / M_sun, X * R_star / au)
        return ["%.1f" % z for z in V]
    xtickslocs = ax0.get_xticks()    
    new_tick_locations = xtickslocs[1:-1]
    #print(new_tick_locations)
    #print(type(new_tick_locations))
    #print(tick_function(new_tick_locations))
...
ax0.set_ylabel(r"Flux density [mJy]")
